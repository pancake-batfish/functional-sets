...
class FunSetSuite extends FunSuite {
  ...
  test("intersection contains only elements in both sets") {
    new TestSets {
      ...
      assert(contains(i, 1), "Intersection 3")
      assert(!contains(i, 2), "Intersection 4")
    }
  }
  ...
  test("diff contains only elements in s but not t") {
    new TestSets {
      ...
      val u = union(s1, s2)
      val d = diff(u, s1)
      assert(!contains(d, 1), "Diff 3")
      assert(contains(d, 2), "Diff 4")
    }
  }

  test("filter includes only elements for which a predicate holds") {
    new TestSets {
      val u = union(s1, s2)
